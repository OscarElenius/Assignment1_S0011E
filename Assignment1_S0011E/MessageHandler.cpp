...
bool MessageDispatcher::discharge(Entity* receiver, const Telegram& msg) {
	return receiver->handleMsg(msg);
}

bool MessageDispatcher::dispatchMessage(double delay, int sender, int receiver, int msg, void* extraInfo) {
	...
	if (delay <= 0.0) { return discharge(pReceiver, telegram); }
	else {
...
void MessageDispatcher::dispatchDelayedMessages() {
	...
	while (!priorityQueue.empty() && (priorityQueue.begin()->dispatchTime < currentTime) && (priorityQueue.begin()->dispatchTime > 0)) {
